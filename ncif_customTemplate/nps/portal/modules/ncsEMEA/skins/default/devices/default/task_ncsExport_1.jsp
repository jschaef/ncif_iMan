...
//			|| (  "xml".equalsIgnoreCase(	par_format )) )			response.setHeader( "Content-disposition","attachment;filename=export." + par_format );
...
</c:when><c:otherwise><% if ( par_preformatted.equalsIgnoreCase( "TRUE" )) { // already formatted - display as is %>
<c:forEach var='dataRow' varStatus="row" items='${pageVar}'><c:out value='${dataRow}' escapeXml='false'/></c:forEach>
<% } else { %><c:forEach var='dataRow' varStatus="row" items='${pageVar}'><c:forEach var='dataCell' varStatus="col" items='${dataRow}'><%= par_quote %><c:out value='${dataCell}' escapeXml='false'/><%= par_quote %><c:if test="${not col.last}"><%= par_separator %></c:if></c:forEach>
